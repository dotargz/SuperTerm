...
class System(pygame.sprite.Sprite):
    ...
    def get_env(self, key, default=None):
        if self.env_cache.get(key) == None:
            try:
                with open(resource_path("assets/data/environment.json"), "r") as f:
                    env = json.load(f)
            except:
                env = {}
            try:
                self.env_cache[key] = env[key]
            except KeyError:
                self.env_cache[key] = default
        return str(self.env_cache[key])

    def dprint(self,*arg):
        if self.get_env("debug", "false").lower() == "true":
            print(*arg)
        ...
        for cmd in tokenized:
            ...
            if command in self.programs:
                ...
                try:
                    ...
                    return_text = f"Error: Program '{command}' failed to run.\n"
...
while Running:
        ...
        if INBIOS == False:
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_RETURN:
                    ...
                    TERMINAL.current_input = ""
                elif event.key == pygame.K_TAB or event.key == pygame.K_CAPSLOCK or event.key == pygame.K_NUMLOCK or event.key == pygame.K_SCROLLOCK or event.key == pygame.K_RSHIFT or event.key == pygame.K_LSHIFT or event.key == pygame.K_RCTRL or event.key == pygame.K_LCTRL or event.key == pygame.K_RALT or event.key == pygame.K_LALT or event.key == pygame.K_LEFT or event.key == pygame.K_RIGHT or event.key == pygame.K_HOME or event.key == pygame.K_END or event.key == pygame.K_PAGEUP or event.key == pygame.K_PAGEDOWN or event.key == pygame.K_F1 or event.key == pygame.K_F2 or event.key == pygame.K_F3 or event.key == pygame.K_F4 or event.key == pygame.K_F5 or event.key == pygame.K_F6 or event.key == pygame.K_F7 or event.key == pygame.K_F8 or event.key == pygame.K_F9 or event.key == pygame.K_F10 or event.key == pygame.K_F11 or event.key == pygame.K_F12 or event.key == pygame.K_INSERT or event.key == pygame.K_PRINTSCREEN or event.key == pygame.K_SCROLLLOCK or event.key == pygame.K_PAUSE or event.key == pygame.K_UNKNOWN or event.key == pygame.K_DELETE or pygame.key.name(event.key) == "VolumeUp" or pygame.key.name(event.key) == "VolumeDown" or pygame.key.name(event.key) == "VolumeMute":
                    pass
    ...
    # BIOS.dprint("FPS: " + str(FramePerSec.get_fps()))
